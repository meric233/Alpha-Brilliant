...
- **App Check enforcement** — *deferred (must-do before public launch).* AI Logic currently runs without App Check for local testing; a TODO in `src/lib/ai/client.ts` documents wiring reCAPTCHA Enterprise before exposing the Gemini endpoint publicly.
